...
public class AltarBlockEntity extends BlockEntity implements ImplementedInventory, ExtendedScreenHandlerFactory<BlockPos> {
    ...
    public float getRenderingRotation() {
        rotation += (float) getStack(0).getCount() / 10;
        if(rotation >= 360) {
            rotation = 0;
        }
        return rotation;
    ...
    public Text getDisplayName() {
        return Text.translatable("block.tutorialmod.altar");
    }

    @Nullable
    @Override
    public ScreenHandler createMenu(int syncId, PlayerInventory playerInventory, PlayerEntity player) {
        return new AltarScreenHandler(syncId, playerInventory, this.pos);
    }
